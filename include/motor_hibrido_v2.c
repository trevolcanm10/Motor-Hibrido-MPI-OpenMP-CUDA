...
int main(int argc, char **argv) {
    ...
    for (int i = world_rank; i < num_archivos; i += world_size) {
        ...
        if (!fp) {
            #pragma omp critical
            printf("[%s MPI %d] No se pudo abrir %s\n", hostname, world_rank, archivos[i]);
            continue;
        }
        fclose(fp); // Cerramos aquí, se reabrirá dentro de OpenMP

        // Procesar el archivo en paralelo por hilos OpenMP
